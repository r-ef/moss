...
func (a *segment) doSort() {
	...
	// Unsafe cast to slice of kvEntry
	entries := unsafe.Slice((*kvEntry)(unsafe.Pointer(&a.kvs[0])), len(a.kvs)/2)

	slices.SortStableFunc(entries, func(i, j kvEntry) int {
		kxLength := int((maskKeyLength & i.opLen) >> 32)
		kxStart := int(i.offset)
		kx := a.buf[kxStart : kxStart+kxLength]

		kyLength := int((maskKeyLength & j.opLen) >> 32)
		kyStart := int(j.offset)
